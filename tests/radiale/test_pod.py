...
from bcoding import bencode # _write_ uses this
...
def test_eprint(mock_flush, mock_write):
    ...
    mock_write.assert_called_once_with(sample_string.encode('utf-8') + '\n'.encode('utf-8'))
...
async def test_outgoing_q_start():
    q = OutgoingQ()
    with patch('asyncio.Queue', return_value=AsyncMock()) as mock_async_queue, \
...
async def test_outgoing_q_out_task_single_item():
    ...
    q.outgoing.get = AsyncMock(return_value={"data": "test"}) # Mock get to return one item then stop
    q.running = True
    ...
    with patch('asyncio.get_event_loop') as mock_loop, \
         patch('radiale.pod._write_') as mock_internal_write:
        ...
        mock_executor.assert_called_once_with(None, mock_internal_write, {"data": "test_item"})
...
@patch('radiale.pod.remap', side_effect=lambda x, visit: x) # Simple pass-through for remap
...
def test_outgoing_q_write_msg(mock_json_dumps, mock_remap):
    ...
    msg_id = "123"
    msg_data = {"some": "data", "nan_val": float('nan')} # Include a NaN to test clean_data via remap
    ...
    mock_json_dumps.assert_called_once_with(msg_data) # json.dumps is called with the original dict
...
async def test_radiale_pod_invoke_sleep_ms(mock_pod):
    ...
    with patch('asyncio.sleep', new_callable=AsyncMock) as mock_async_sleep:
        ...
        mock_pod.out.write_msg.assert_called_once_with(id="sleep1", status="done", data=500)
...
async def test_radiale_pod_invoke_astral_now(mock_pod, location_data):
    ...
    with patch('radiale.schedule.astral_now', return_value=expected_period) as mock_schedule_astral_now:
        await mock_pod.invoke(msg)

        mock_schedule_astral_now.assert_called_once_with(**opts_dict)
        mock_pod.out.write_msg.assert_called_once_with(id="astral1", status="done", data=expected_period)
...
async def test_radiale_pod_run_pod_handles_describe():
    ...
    with patch('asyncio.get_event_loop') as mock_loop, \
         patch('radiale.pod.bdecode', new_callable=lambda: async_bdecode_mock): # Patch bdecode

        # Mock run_in_executor for bdecode
        mock_loop.return_value.run_in_executor = AsyncMock(side_effect=async_bdecode_mock)
    ...
    expected_describe_output = describe_this([
        'listen-mdns', 'listen-mqtt', 'listen-deconz', 'subscribe-chromecast',
        'millis-solar', 'millis-crontab', 'put-deconz', 'mdns-info',
        'subscribe-esp', 'switch-esp', 'service-esp', 'light-esp', 'state-esp'
    ])
...
async def test_radiale_pod_run_pod_handles_invoke_error(mock_pod):
    ...
    invoke_msg = {"op": "invoke", "var": "some/failing*fn", "id": "fail1", "args": invoke_msg_args}
    ...
    with patch('asyncio.get_event_loop') as mock_loop, \
         patch('radiale.pod.bdecode', new_callable=lambda: async_bdecode_mock), \
         patch('radiale.pod.eprint') as mock_eprint: # Also mock eprint
        ...
        with patch('radiale.pod.OutgoingQ') as MockOutgoingQInstance:
            MockOutgoingQInstance.return_value.start = AsyncMock(return_value=mock_pod.out)
...
async def test_radiale_pod_invoke_listen_mdns(mock_pod):
    ...
    with patch('radiale.pod.mdns') as mock_mdns_module:
        ...
        mock_mdns_module.MDNS.return_value.start = AsyncMock(return_value=mock_mdns_service_instance)
