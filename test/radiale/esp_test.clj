...
(deftest keywordize-esp-services-test
  (let [device-name "my-esp-device"
        services    {"12345" {:object_id "light_1"
                              :name      "Main Light"
                              :key       12345} ; ESPHome service
                     "67890" {:name "custom_svc"
                              :key  67890}}] ; User-defined service
    (testing "Keywordizing ESPHome and user-defined services"
      (let [result (esp/keywordize-esp-services device-name services)]
        (is
          (contains? result :light_1))
        (is
          (= :light_1 (get result "12345")))
        (is
          (= {:props {:object_id "light_1"
                      :name      "Main Light"
                      :key       12345}}
             (get result :light_1)))

        (is
          (contains? result :custom_svc))
        (is
          (= :custom_svc (get result "67890")))
        (is
          (= {:props {:name "custom_svc"
                      :key  67890}}
...
(deftest esp-logger-test
  (let [bus    (chan) ; Not directly used by all paths, but part of signature
    ...
    (testing "Receiving services"
      (reset! state* {})
      (let [services-payload {"abc" {:object_id "switch1"
                                     :name      "My Switch"}
                              "def" {:name "my_user_service"}}
            msg {:service-name "test-esp"
                 :services     services-payload}]
        (with-redefs [esp/keywordize-esp-services (fn [dev-name srvs]
                                                    {:mocked_switch1  {:props (srvs "abc")}
                                                     :mocked_user_svc {:props (srvs "def")}})]
          (esp/esp-logger bus state* base-m msg))
        (is
          (= {:mocked_switch1  {:props {:object_id "switch1"
                                        :name      "My Switch"}}
    ...
    (testing "Device connection status changed"
      ...
      (esp/esp-logger
        ...
        state*
        base-m
        {:service-name "test-esp"
         :connected    true})
      (is
        (true? (get-in @state* [:radiale.esp :test-esp :connected])))
      (esp/esp-logger
        bus
        state*
        base-m
        {:service-name "test-esp"
         :connected    false})
      (is
        (false? (get-in @state* [:radiale.esp :test-esp :connected]))))
    ...
    (testing "Regular state update"
      ...
      (esp/esp-logger
        ...
        {:service-name "test-esp"
         :state        ["111" "ON"]})
      (is
        (= "ON" (get-in @state* [:radiale.esp :test-esp :the_switch :state]))))
    (close! bus)))
...
(deftest discover-test
  (let [bus (chan 10)
        ...
        mock-radiale-map {:listen-mdns   (fn [opts cb]
                                           (swap! listen-mdns-calls conj
                                             {:opts opts
                                              :cb   cb}))
                          :subscribe-esp (fn [opts cb]
                                           (swap! subscribe-esp-calls conj
                                             {:opts opts
                                              :cb   cb}))}]

    (with-redefs [esp/esp-logger (fn [b s* m_orig esp_msg]
                                   (swap! esp-logger-calls conj
                                     {:esp_msg esp_msg
                                      :m_orig  m_orig}))]
      ...
      (let [{:keys [opts cb]} (first @listen-mdns-calls)]
        (is
          (= {:service-type "_esphomelib._tcp.local."} opts))
        ...
        (testing "MDNS 'added' event triggers subscribe-esp"
          (cb
            {:state-change "added"
             :service-name "esp1"})
          (is
            (= 1 (count @subscribe-esp-calls)))
          (let [{subscribe-opts :opts
                 subscribe-cb   :cb}
                (first @subscribe-esp-calls)]
            (is
              (= {:service-name "esp1"} subscribe-opts))
            ...
            (let [esp-device-services {:services {"s1" {:name "light"}}}]
              ...
              (is
                (= 1 (count @esp-logger-calls)))
              (is
                (= esp-device-services (:esp_msg (first @esp-logger-calls))))
              (is
                (= discover-m (:m_orig (first @esp-logger-calls)))))))
        ...
        (testing "MDNS 'updated' event triggers subscribe-esp"
          ...
          (let [{subscribe-opts :opts
                ...
                (first @subscribe-esp-calls)]
            (is
              (= {:service-name "esp2"} subscribe-opts))
            (subscribe-cb {:connected true})
            (is
              (= {:connected true} (:esp_msg (first @esp-logger-calls))))))
...
(deftest command-functions-test
  (let [bus (chan 10)
        ...
        mock-esp-base-data-val {:service-name "test-esp"
                                :key          "entity_key_from_base"}
        ...
        common-test-fn (fn [command-fn pod-fn-kw cmd-specific-payload expected-pod-payload]
                         ...
                         (let [mock-pod-fn        (fn [params cb]
                                                    (swap! mock-pod-fn-calls conj params)
                                                    (cb {:success true}))
                           ...
                           (is
                             (= (merge mock-esp-base-data-val expected-pod-payload) (first @mock-pod-fn-calls)))
                           (is
                             (= message-payload (poll! bus)))))]

    (testing "esp/switch"
      (common-test-fn
        esp/switch
        :switch-esp
        {::esp/ident :test-esp/switch1
         ::esp/state true}
        {:state true}))
