      ...
      (let [er (:radiale.esp @state*)]
        (cond
          ...
          (swap! state* update-in
            (into [:radiale.subscription] ha-state-subscribe)
            clojure.set/union
            (set [:radiale.esp service-name]))
...
(defn discover
  ...
  (listen-mdns
    ...
    (fn [{:keys [state-change service-name]
          :as   mdns-state}]
      (when (or (= state-change "updated") (= state-change "added"))
        (subscribe-esp {:service-name service-name} (partial esp-logger bus state* m))))))
...
(defn switch
  [{:keys [switch-esp]} bus state*
   {:keys [::ident ::state]
    :as   m}]
  (switch-esp
    (merge (esp-base-data state* ident) {:state state})
    (fn [r]
      (prn r)))
  (async/>!! bus m))
...
(defn service
  [{:keys [service-esp]} bus state*
   {:keys [::ident ::params]
    :as   m}]

  (service-esp
    (merge (esp-base-data state* ident) {:params params})
    (fn [r]
...
(defn state
  [{:keys [state-esp]} bus state*
   {:keys [::ident ::entity-id ::attribute ::state]
    :as   m}]
