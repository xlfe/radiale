...
def describe_this(fn_names):
    ...
    return {
        "format": "json",
        "ops": {"shutdown": {}},
        "namespaces": [{"name": "pod.xlfe.radiale", "vars": d}],
    }
...
class OutgoingQ:
    ...
    def write_msg(self, id, data, status="status"):
        self.write_raw(
            dict(
                value=json.dumps(
                    remap(data, visit=clean_data) if type(data) is dict else data
                ),
                id=id,
                status=[status],
            )
    ...
    async def invoke(self, msg):
        ...
        if var.endswith("sleep-ms"):
            ...
            if "mdns" not in self.services:
                self.services["mdns"] = await mdns.MDNS().start(self.out)
            ...
            asyncio.create_task(self.services["deconz"].listen(self.out, id, opts))
            ...
            asyncio.create_task(
                self.services["deconz"].put(
                    self.out, id, opts, type_name, device_id, state
                )
            )

        elif var.endswith("listen-mqtt*"):
            asyncio.create_task(mqtt.mqtt_listen(self.out, id, opts))
            ...
            if svc is None:
                ...
                self.chromecast[sn] = svc = chromecast.Chromecast(
                    self.out, id, _mdns, sn
                )
            ...
            service = self.esp.get(sn)
            assert service
            await service.state_update(
                id, opts["entity_id"], opts["attribute"], opts["state"]
            )
